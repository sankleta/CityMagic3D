...
class VoxelizeCollate:
    ...
    def __call__(self, batch):
        ...
        return voxelize(batch, self.ignore_label, self.voxel_size, self.probing, self.mode,
                        ...
                        num_queries=self.num_queries, input_mode=self.input_mode)
...
class VoxelizeCollateMerge:
    ...
    def __call__(self, batch):
        if (
            ...
            batch = new_batch
        # return voxelize(batch, self.ignore_label, self.voxel_size, self.probing, self.mode)
        return voxelize(batch, self.ignore_label, self.voxel_size, self.probing, self.mode, task=self.task, input_mode=self.input_mode)
...
def voxelize(batch, ignore_label, voxel_size, probing, mode, task,
             ignore_class_threshold, filter_out_classes, label_offset, num_queries, input_mode='full_dataset'):
    ...
    if mode == "test" and input_mode=='single_scene':
        pass
    elif mode == "test":
